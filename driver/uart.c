...
void ICACHE_FLASH_ATTR myprintf(const char *format, ...)
{
#define MaxBuff 1024
    unsigned char buffer[MaxBuff];
    va_list al;
    va_start(al, format);
    ets_vsnprintf((char*) buffer, MaxBuff - 1, format, al);
    va_end(al);
    uart0_tx_buffer(buffer, os_strlen((char*) buffer));
    // Also send the debug output to our websocket website
    websockDebugOut(buffer);
}
